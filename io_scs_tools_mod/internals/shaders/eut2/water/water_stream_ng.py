...
def __create_node_group__():
    ...
    # inputs defining
    yaw0_socket = water_stream_ng.interface.new_socket(name = "Yaw0", in_out='INPUT', socket_type = 'NodeSocketVector')
    yaw0_socket.attribute_domain = 'POINT'

    speed0_socket = water_stream_ng.interface.new_socket(name = "Speed0", in_out='INPUT', socket_type = 'NodeSocketFloat')
    speed0_socket.attribute_domain = 'POINT'

    yaw1_socket = water_stream_ng.interface.new_socket(name = "Yaw1", in_out='INPUT', socket_type = 'NodeSocketVector')
    yaw1_socket.attribute_domain = 'POINT'

    speed1_socket = water_stream_ng.interface.new_socket(name = "Speed1", in_out='INPUT', socket_type = 'NodeSocketFloat')
    speed1_socket.attribute_domain = 'POINT'

    input_n = water_stream_ng.nodes.new("NodeGroupInput")
    input_n.location = (start_pos_x - pos_x_shift, start_pos_y)

    # outputs defining
    stream0_socket = water_stream_ng.interface.new_socket(name = "Stream0", in_out='OUTPUT', socket_type = 'NodeSocketVector')
    stream0_socket.attribute_domain = 'POINT'

    stream1_socket = water_stream_ng.interface.new_socket(name = "Stream1", in_out='OUTPUT', socket_type = 'NodeSocketVector')
    stream1_socket.attribute_domain = 'POINT'
