...
import bpy
from io_scs_tools_mod.internals.shaders.eut2.std_node_groups import linear_to_srgb_ng
...
def __create_vcolor_group__():
    ...
    # outputs defining
    vertex_color_socket = vcol_g.interface.new_socket(name = "Vertex Color", in_out='OUTPUT', socket_type = 'NodeSocketColor')
    vertex_color_socket.attribute_domain = 'POINT'

    vertex_color_alpha_socket = vcol_g.interface.new_socket(name = "Vertex Color Alpha", in_out='OUTPUT', socket_type = 'NodeSocketFloat')
    vertex_color_alpha_socket.attribute_domain = 'POINT'

    output_n = vcol_g.nodes.new("NodeGroupOutput")
    output_n.location = (185 * 5, 0)
    output_n.is_active_output = True
