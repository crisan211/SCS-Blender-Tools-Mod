...
def __create_node_group__():
    ...
    # inputs defining
    alpha_socket = asafew_g.interface.new_socket(name = "Alpha", in_out = 'INPUT', socket_type = 'NodeSocketFloat')
    alpha_socket.attribute_domain = 'POINT'

    factor1_socket = asafew_g.interface.new_socket(name = "Factor1", in_out = 'INPUT', socket_type = 'NodeSocketFloat')
    factor1_socket.attribute_domain = 'POINT'

    factor2_socket = asafew_g.interface.new_socket(name = "Factor2", in_out = 'INPUT', socket_type = 'NodeSocketFloat')
    factor2_socket.attribute_domain = 'POINT'

    input_n = asafew_g.nodes.new("NodeGroupInput")
    input_n.location = (0, 0)

    # outputs defining
    weighted_alpha_socket = asafew_g.interface.new_socket(name = "Weighted Alpha", in_out = 'OUTPUT', socket_type = 'NodeSocketFloat')
    weighted_alpha_socket.attribute_domain = 'POINT'
