...
def __create_linear_to_srgb_group__():
    ...
    # inputs defining
    lin_to_srgb_g_in_socket = lin_to_srgb_g.interface.new_socket(name = "Value", in_out='INPUT', socket_type = 'NodeSocketFloat')
    lin_to_srgb_g_in_socket.attribute_domain = 'POINT'

    input_n = lin_to_srgb_g.nodes.new("NodeGroupInput")
    input_n.location = (start_pos_x - pos_x_shift, 0)

    # outputs defining
    lin_to_srgb_g_out_socket = lin_to_srgb_g.interface.new_socket(name = "Value", in_out='OUTPUT', socket_type = 'NodeSocketFloat')
    lin_to_srgb_g_out_socket.attribute_domain = 'POINT'
    ...
    # group links
    # pass: 1
    lin_to_srgb_g.links.new(input_n.outputs[0], big_pow_n.inputs[0])

    # pass: 2
    lin_to_srgb_g.links.new(big_mult_n.inputs[0], big_pow_n.outputs[0])

    # pass: 3
    lin_to_srgb_g.links.new(small_mult_n.inputs[0], input_n.outputs[0])
    lin_to_srgb_g.links.new(big_sub_n.inputs[0], big_mult_n.outputs[0])

    # pass: 4
    lin_to_srgb_g.links.new(is_small_n.inputs[0], input_n.outputs[0])
    lin_to_srgb_g.links.new(is_big_n.inputs[0], input_n.outputs[0])

    # pass: 5
    lin_to_srgb_g.links.new(small_factor_n.inputs[0], is_small_n.outputs[0])
    lin_to_srgb_g.links.new(small_factor_n.inputs[1], small_mult_n.outputs[0])

    lin_to_srgb_g.links.new(big_factor_n.inputs[0], is_big_n.outputs[0])
    lin_to_srgb_g.links.new(big_factor_n.inputs[1], big_sub_n.outputs[0])

    # pass: 6
    lin_to_srgb_g.links.new(final_mix_n.inputs[0], small_factor_n.outputs[0])
    lin_to_srgb_g.links.new(final_mix_n.inputs[1], big_factor_n.outputs[0])

    # pass: out
    lin_to_srgb_g.links.new(output_n.inputs[0], final_mix_n.outputs[0
                                                                    ])
