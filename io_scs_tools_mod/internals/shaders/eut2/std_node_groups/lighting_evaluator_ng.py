...
def __create_group__():
    ...
    if LIGHTING_EVALUATOR_G not in bpy.data.node_groups:  # creation
        ...
        # inputs defining
        normal_vector_socket = lighting_eval_g.interface.new_socket(name = "Normal Vector", in_out='INPUT', socket_type = 'NodeSocketVector') # n_normal
        normal_vector_socket.attribute_domain = 'POINT'

        incoming_vector_socket = lighting_eval_g.interface.new_socket(name = "Incoming Vector", in_out='INPUT', socket_type = 'NodeSocketVector')  # n_eye
        incoming_vector_socket.attribute_domain = 'POINT'

        shininess_socket = lighting_eval_g.interface.new_socket(name = "Shininess", in_out='INPUT', socket_type = 'NodeSocketFloat') # specular_exponent
        shininess_socket.attribute_domain = 'POINT'
        
        flat_lightning_socket = lighting_eval_g.interface.new_socket(name = "Flat Lighting", in_out='INPUT', socket_type = 'NodeSocketFloat') # flat lighting switch, should be 0 or 1
        flat_lightning_socket.attribute_domain = 'POINT'

        # outputs defining
        diffuse_lightning_socket = lighting_eval_g.interface.new_socket(name = "Diffuse Lighting", in_out='OUTPUT', socket_type = 'NodeSocketColor') # final diffuse lighting
        diffuse_lightning_socket.attribute_domain = 'POINT'

        specular_lightning_socket = lighting_eval_g.interface.new_socket(name = "Specular Lighting", in_out='OUTPUT', socket_type = 'NodeSocketColor') # final specular lighting
        specular_lightning_socket.attribute_domain = 'POINT'

        normal_socket = lighting_eval_g.interface.new_socket(name = "Normal", in_out='OUTPUT', socket_type = 'NodeSocketVector') # bypassed normal, to have one access point to final normal
