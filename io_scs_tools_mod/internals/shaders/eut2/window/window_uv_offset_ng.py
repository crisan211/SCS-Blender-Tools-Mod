...
from io_scs_tools_mod.internals.shaders.eut2.std_node_groups import scs_uvs_combine_ng
from io_scs_tools_mod.internals.shaders.eut2.std_node_groups import scs_uvs_separate_ng
from io_scs_tools_mod.internals.shaders.eut2.window import window_offset_factor_ng
from io_scs_tools_mod.internals.shaders.eut2.window import window_final_uv_ng
...
def __create_node_group__():
    ...
    # inputs defining
    uv_socket = uv_offset_g.interface.new_socket(name = "UV", in_out='INPUT', socket_type = 'NodeSocketVector')
    uv_socket.attribute_domain = 'POINT'

    normal_socket = uv_offset_g.interface.new_socket(name = "Normal", in_out='INPUT', socket_type = 'NodeSocketVector')
    normal_socket.attribute_domain = 'POINT'

    incoming_socket = uv_offset_g.interface.new_socket(name = "Incoming", in_out='INPUT', socket_type = 'NodeSocketVector')
    ...
    # outputs defining
    uv_final_socket = uv_offset_g.interface.new_socket(name = "UV Final", in_out='OUTPUT', socket_type = 'NodeSocketVector')
    uv_final_socket.attribute_domain = 'POINT'
