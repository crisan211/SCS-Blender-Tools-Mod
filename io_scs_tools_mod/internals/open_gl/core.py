...
from io_scs_tools_mod.internals.open_gl import locators as _locators
from io_scs_tools_mod.internals.open_gl import primitive as _primitive
from io_scs_tools_mod.internals.open_gl.cache import LocatorsCache
...
from io_scs_tools_mod.utils.printout import get_immediate_msg as _get_immediate_msg
...
def _draw_3dview_report(window, area, region):
    ...
    :type region: bpy.types.Region
    """
    from io_scs_tools_mod.operators.wm import SCS_TOOLS_OT_Show3DViewReport as _Show3DViewReport
    ...
    # draw BT banner
    (img, width, height) = _Show3DViewReport.get_scs_banner_img_data(window)
    ...
    batch = batch_for_shader(
        shader, 'TRI_FAN',
        {
            "pos": (
                (pos_x - 5, pos_y), 
                (pos_x - 5 + width, pos_y), 
                (pos_x - 5 + width, pos_y + height), 
                (pos_x - 5, pos_y + height)
            ),
