...
def _get_look(section):
    ...
    for sec in section.sections:
        if sec.type == "Material":
            ...
            # Extra treatment for window shaders
            #
            # If day or night version of it is detected, switch it to window.lit.
            if mat_effect.startswith("eut2.window") and mat_effect.endswith(".day") or mat_effect.endswith(".night"):

                mat_effect = mat_effect.replace(".day", ".lit")
                mat_effect = mat_effect.replace(".night", ".lit")

            # Lightmap
            if mat_effect.startswith("eut2.lightmap.lit"):

                mat_effect = mat_effect.replace(".lightmap.night", ".lightmap.night")
